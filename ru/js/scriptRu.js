...
//user name
const nameInput = document.querySelector('[placeholder="[Введите имя]"]');
...
window.onload = function () {
  ...
  city.value = window.localStorage.getItem("city") || "Минск";
  showWeather();
...
const temperature = document.querySelector(".temperature");
const weatherDescription = document.querySelector(".weather-description");
let city = document.querySelector('[placeholder="Введите город"]');
let wind = document.querySelector(".wind");
let humidity = document.querySelector(".humidity");
let weatherError = document.querySelector(".weather-error");
...
function showWeather() {
  ...
  request.open(
    "GET",
    `https://api.openweathermap.org/data/2.5/weather?q=${city.value}&lang=ru&appid=3dd91f1c4372960bfa4e35f08539b09c&units=metric`,
    true
  );
  ...
  request.onload = function () {
    ...
    if (this.status == 200) {
      ...
      if (city.value == "") {
        weatherError.textContent = "Ошибка: Вы ввели пустую строку!";
      } else {
        weatherError.textContent = "Ошибка: Вы ввели неверный город!";
      }
...
function showQuoteOfTheDay() {
  ...
  quotesRequest.open("GET", "js/quotesRu.json", true);
